...
void dfs(const vector<vector<int>>& grid, const pair<int, int>& start, const pair<int, int>& end) {
    int rows = grid.size();
    int cols = grid[0].size();
    ...
    stack<Point> st;
    st.push({start.first, start.second, 0, {start}});
    ...
    while (!st.empty()) {
        ...
        for (const auto& move : moves) {
            ...
            if (isValidMove(grid, visited, newRow, newCol)) {
                ...
                st.push({newRow, newCol, currentDistance + 1, newPath});
